package com.coderknot.gamesearch;

public class Constants {

    // Cheapshark API
    public static final String CHEAPSHARK_BASE_URL = "http://www.cheapshark.com/api/1.0/";
    public static final String CHEAPSHARK_RESOURCE_GAMES =  "games";
    public static final String CHEAPSHARK_RESOURCE_DEALS = "deals";
    public static final String CHEAPSHARK_TITLE_PARAMETER = "title";
    public static final String CHEAPSHARK_SORTBY_PARAMETER = "sortBy";
    public static final String CHEAPSHARK_DESC_PARAMETER = "desc";

}
